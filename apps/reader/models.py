...
class Rating(models.Model):
    ...
    def rating(self):
        return sum([
            x.field.weight * (x.number - 1) / 4
            for x in self.ratingresponse_set.all()
        ])

    def __getattr__(self, key):
        if key.startswith("field_"):
            resp = RatingResponse.objects.filter(
                rating=self, field__name=key.rsplit("_", 1)[-1]).first()
    ...
    def __setattr__(self, key, value):
        if key.startswith("field_") and not key.endswith("_state"):
            ...
            RatingResponse.objects.create(
                rating=self, field=rating_field, number=value)
            return
