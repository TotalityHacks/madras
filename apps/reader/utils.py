...
def get_metrics_github(github_user_name):
	...
	try:
		...
		return {"num_followers": user.followers_count, "num_repos": count_of_valid_repos, "num_contributions": get_contributions(github_user_name), "self_star_repos": len(set(list(user_repos)).intersection(list(user_starred)))}
